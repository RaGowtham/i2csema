...
static int adl_bmc_vm_probe(struct platform_device *pdev)
{
	...
	adl_bmc_initdata.driver_data = vm_data;
	...
	for (i = 0; i < 16; i++)
	{
		...
		vm_data->adl_bmc_vm_desc[i].owner = THIS_MODULE;
	...
	for (i = 0; i < vm_data->cnt; i++) 
        {
		vm_data->adl_bmc_vm_rdev[i] = regulator_register(&vm_data->adl_bmc_vm_desc[i], &pdev->dev, &adl_bmc_initdata, vm_data);
